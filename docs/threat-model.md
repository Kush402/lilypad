...
| Ask's instant step (TypeSafe)    | A second third party, only with the person's own TypeSafe key on the Mac ([ADR-0019](adr/0019-ask-does-short-commands-instantly.md)). It receives the command, the app in front and the names of on-screen controls, never a screenshot, field values or window titles; the phone discloses it as its own destination and only a command echoing that consent revision may use it. It can only choose among options Ask offered, above fixed thresholds, and its choice passes the same floor and autonomy gate as the model's. A page label written to steer it can at most pick one of that page's own controls, and only one whose label shares a word with the command; in live probes (`live_steering_labels_never_get_the_click`) no planted label was ever chosen. The model version is pinned to the one its thresholds were measured on. |
