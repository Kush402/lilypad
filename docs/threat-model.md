...
| Ask acting beyond its grant                   | Ask's gestures run on the session's own input thread behind the same scope, liveness and Accessibility gates as the phone ([ADR-0018](adr/0018-ask-operates-the-mac-under-full-control.md)). Full control is opt-in per Mac and per run; a floor refuses password fields, Lilypad itself, security and permission prompts, lock/log-out shortcuts and dangerous terminal commands in every mode; screen text is framed to the model as untrusted content; a touch on the phone or at the Mac takes over at once. See [ask-computer-use.md](ask-computer-use.md).                                                                                                                                                                                                                                                                                                                                          |
| Hosted Ask service-key theft or free-tier use | The TypeSafe service credential exists only in backend environment, is sent only over HTTPS with redirects refused, and never enters a client or log. `POST /ask/v1/systemone` requires a live device token plus effective Pro/Team entitlement and fails closed when entitlement, Redis metering, configuration, or upstream service is unavailable. Strict text-only schemas reject screenshots and unknown fields; only expiring task/step counters are stored. When accessibility exposes no actionable controls, Apple Vision OCR stays local: only a complete recognized label already present in the person's command may enter the request, rebuilt from the command rather than copied from the screen, and visual targets are bound to the captured pixels. See [ADR-0020](adr/0020-lilypad-runs-computer-use-on-its-own-account.md) and [ADR-0022](adr/0022-hosted-ocr-is-local-targeting.md). |
| Ask's instant step (TypeSafe)                 | A second third party, only with the person's own TypeSafe key on the Mac ([ADR-0019](adr/0019-ask-does-short-commands-instantly.md)). It receives the command, the app in front, the role and label of on-screen actionable controls (including buttons, links, fields, rows and menu items), and installed-app names sharing a word with the command; never a screenshot, field values or window titles. The phone discloses it as its own destination and only a command echoing that consent revision may use it. It can only choose among options Ask offered, above fixed thresholds; code additionally binds controls and shortcuts to the person's words, and every choice passes the same floor and autonomy gate as the model's. Ambiguous control names fall back to the language model. The model version is pinned to the one its thresholds were measured on.                                |
