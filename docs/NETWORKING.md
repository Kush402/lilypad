...
| Control plane              | **Dual path.** Cloud: unchanged (`/pairing/*`, `/connect/request`, `/ws/signal`). **LAN:** embedded TLS server on the desktop (`https://<lan-ip>:8787`, default) serves `/health`, `/connect/request`, and `/ws/signal` with a 2-peer hub. Trust cache authorizes reconnect via `connect_secret_hash` from `trust-record` (incremental) and `trust-sync` (full replace on presence rejoin / revoke — omissions revoke). Mobile races LAN before cloud when `lanTlsCertSha256` is cached. The desktop takes its own seat in a LAN room **in-process** (`lan::loopback`) — see below. |
| Internet needed on LAN?    | **No (control path).** Automated test `lan_control_e2e` drives the whole handshake against 127.0.0.1 with zero cloud: `/health`, `/connect/request`, a pinned-TLS phone WebSocket, `pair-request`, and `session-start`. End-to-end offline video/input on real hardware is not yet CI-gated. Release builds still default to `https://api.takedia.com` for cloud rendezvous.                                                                                                                                                                                                        |
...
### Why the desktop does not use a socket to reach its own LAN hub

The desktop is the LAN control plane, so when a phone rings over the LAN the
room is minted on `wss://<lan-ip>:8787/ws/signal` — this machine's own address,
served with the self-signed certificate the phone pins. Joining that room the
way the cloud path joins a cloud room means opening a WebSocket to itself, and
the desktop's client verifies against webpki roots: the handshake fails, the
desktop never takes its seat, and the phone's `pair-request` is relayed to
nobody. The phone shows "Waiting for approval…" indefinitely and the laptop
never rings.

That is exactly what shipped in v0.1.20. It was invisible until then because the
_first_ ring goes over the cloud (a phone has no LAN cache yet), and the second
ring only reached the LAN path once trust-record re-sync stopped the LAN
`/connect/request` from 404ing.

The fix is `lan::loopback`: `LanHub` is an in-process object, so the desktop
takes its seat by calling it directly behind the same `SignalingHandle` the
socket transport returns. No TLS, no loopback TCP, no certificate-trust
decision — and one fewer thing outside this process that a LAN session depends
on. The hub also buffers frames for a seat that has not attached yet, because
the phone holds the room id before the desktop has finished tearing down any
previous session.
