...
| PROD-1      | **The account layer is connected on neither end.** The desktop's `auth.rs` was dead code until M9 wired its token path; `SignInScreen.tsx` has no route and zero importers; `approveDesktopEnrollment()`'s only caller is a test; and the desktop has no enrollment-code UI at all (zero hits for `enroll` in `apps/desktop/src/`). A user can install Lilypad, grant permissions and pair a phone without ever having an account — which is also why `authorize.ts` still needs its unowned lane.                                                                                                                                                                                                                                                                                          | P1        |
