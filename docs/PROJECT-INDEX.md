...
| `routes/`    | auth, pairing, signaling, devices, billing, hosted Ask (`POST /ask/v1/systemone`), health            | ✅ hosted Ask is documented in `ask-computer-use.md`               |
...
| Ask computer use      | [ask-computer-use.md](ask-computer-use.md) (instant actions: [ADR-0019](adr/0019-ask-does-short-commands-instantly.md); hosted Pro Jev: [ADR-0020](adr/0020-lilypad-runs-computer-use-on-its-own-account.md)) |
...
| ~~LAN-1~~ | ~~**No LAN discovery of any kind.**~~ **Closed (M9.5):** mDNS advertise (desktop) + browse (iOS/Android), cached LAN URL probe, `NSBonjourServices` / Local Network entitlements. | ✅ done |
| ~~LAN-2~~ | ~~**A session cannot start without a reachable backend**~~ **Closed (M9.5):** embedded TLS control server on desktop serves `/connect/request` + `/ws/signal`; trust cache + `trust-record` for offline auth. Cloud path unchanged. | ✅ done |
| ~~LAN-3~~ | ~~**Presence and no-QR reconnect are cloud-only.**~~ **Closed (M9.5):** `/connect/request` on LAN + trust cache; cloud presence WS retained for remote mode. | ✅ done |
